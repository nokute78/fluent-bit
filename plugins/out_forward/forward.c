...
static int unix_io_write(struct flb_upstream_conn *unused, int fd, const void* data,
                         size_t len, size_t *out_len)
{
    return flb_io_fd_write(fd, data, len, out_len);
}

static int unix_io_read(struct flb_upstream_conn *unused, int fd, void* buf,size_t len)
{
                                 ...
                                 struct flb_config *config)
{
    ...
    if (fc->unix_path) {
        ...
        fc->io_write = unix_io_write;
        fc->io_read  = unix_io_read;
#else
